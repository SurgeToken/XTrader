        ...
        () => {
            async function connectProvider() {
                try {
                    ...
                    if (err === undefined) {
                        alert('If you are having trouble connecting to MetaMask, please check if you still have a pending connection request') //TODO still checking wallet library to catch MetamskError better
                    }
...
function web3EventHandler(web3, setEvent, results, data) {
    if (web3) {
        ...
        if (data.returnValues && (data.returnValues[0] === address || data.returnValues[1] === address)) {
            setEvent(data);
        }
    }
    ...
    useEffect(() => {
        async function getHoldings() {
            ...
            for(let key of Object.keys(contracts)) {
                const contract = new contracts[key](web3, web3.eth.defaultAccount, web3EventHandler.bind(null, web3, setEvent));
                const symbol = await contract.symbol();
                let balance = parseInt(await contract.balanceOf(web3.eth.defaultAccount));
                let decimals = parseInt(await contract.decimals());
                newHoldings[symbol] = {
                    contract,
                    symbol: symbol,
                    balance: (decimals ? balance * .1 ** decimals : balance).toFixed(4),
...
export const Wallet = ({errorHandler, children}) => {
    ...
    return (
        <WalletContext.Provider value={{provider, web3, holdings, connect: setConnect, event}}>
            {children}
